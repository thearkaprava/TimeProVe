...
Unless --instruction is set, the system prompt is built from the OTB action-list file
(OTB activity classes). After inference, class codes from the model are merged with a
fallback: query tokens that match few enough class labels pull in every such action name
(object-in-label). Use --no-otb-fallback to disable merging.
"""
...
# This file lives in <repo>/scripts/; repo root is one level up (for evaluation/, videollama3/, data/).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="VideoLLaMA3 text-only LLM; save I/O to JSON.")
    parser.add_argument(
        "--model-path",
        default=None,
    ...
    if args.input_file is not None:
        user_text = args.input_file.read_text(encoding="utf-8")
    else:
    ...
    if args.instruction.strip():
        ...
        if not args.classes_file.is_file():
            parser.error(f"Classes file not found: {args.classes_file} (or pass --instruction).")
