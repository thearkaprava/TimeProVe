...
2) In order of relevance: a **text-only Gemma** proposes a tentative answer from detected
   ...
   description -> Gemma text-only confidence (sufficient to answer?). If confidence is 1,
   run **GPT final QA** on the same clip, conditioned on the written description, and stop.
   Otherwise continue to the next window.
3) If no window passes confidence, run the same GPT final QA on the last processed
   window's clip and descriptions (fallback to Gemma text-only QA when clip/GPT fails).

Optional: --reuse-intermediate cached clips/descriptions; --skip-confidence-refine uses
only the top-relevance window (one clip + GPT QA, no confidence model).
"""
    ...
    *,
    ...
    if style == PROMPT_STYLE_GENERIC or not q:
        if hint:
            return (
                DESCRIPTION_PROMPT
                + f"\n\nAction context for this clip: {hint}\n"
                "Pay special attention to objects, activities, and interactions mentioned in the action context above."
            )
    ...
    if style == PROMPT_STYLE_QUERY_STRICT:
        ...
        if hint:
            parts += [
                "",
                f"Action context for this clip: {hint}",
                "This context describes the expected activities and objects in the clip. "
                "Pay close attention to these specific elements when writing your description.",
            ]
        parts += [
            "",
...
def discover_videos(video_dir: str, pattern: str) -> list[str]:
    search = os.path.join(video_dir, "**", pattern) if "**" not in pattern else os.path.join(video_dir, pattern)
    paths = sorted(glob.glob(search, recursive=True))
    if not paths:
        alt = os.path.join(video_dir, pattern)
...
def tags_from_basename(basename: str, delimiter: str) -> dict[str, Any]:
    ...
    source_id = parts[0] if parts else None
    return {
        "tags": parts,
...
def run_text_only(
    text_llm_runtime: dict[str, Any],
    instruction: str,
    user_text: str,
    gen_kwargs: dict[str, Any],
) -> str:
    """Text-only inference via Gemma (timeline merge, hints, confidence, QA fallback)."""
    combined_prompt = f"{instruction.strip()}\n\n{user_text.strip()}".strip()
    messages = [
        {
            "role": "user",
    ...
    generation_config = deepcopy(text_llm_runtime["generation_config"])
    if "max_new_tokens" in gen_kwargs and gen_kwargs["max_new_tokens"] is not None:
        generation_config.max_new_tokens = int(gen_kwargs["max_new_tokens"])
    if "do_sample" in gen_kwargs and gen_kwargs["do_sample"] is not None:
        generation_config.do_sample = bool(gen_kwargs["do_sample"])
    if "temperature" in gen_kwargs and gen_kwargs["temperature"] is not None:
        generation_config.temperature = float(gen_kwargs["temperature"])
    if "top_p" in gen_kwargs and gen_kwargs["top_p"] is not None:
        generation_config.top_p = float(gen_kwargs["top_p"])
    response = text_llm_runtime["pipe"](
        text=messages,
...
def _is_gemma4_model_path(model_path: str | None) -> bool:
    mp = str(model_path or "").strip().lower()
    return "gemma-4" in mp


def _pipeline_device_from_map(device_map: str) -> int:
    dm = str(device_map or "").strip().lower()
    if dm in ("", "none", "auto"):
        return 0
    if dm == "cpu":
        return -1
    if dm == "cuda":
        return 0
    m = re.match(r"^cuda:(\d+)$", dm)
    if m:
        return int(m.group(1))
    return 0


def _parse_gpu_ids_csv(gpu_ids_csv: str) -> list[str]:
    return [x.strip() for x in str(gpu_ids_csv or "").split(",") if x.strip()]


def init_text_llm_runtime(args: argparse.Namespace, log: logging.Logger) -> tuple[dict[str, Any], str]:
    gpu_ids = _parse_gpu_ids_csv(getattr(args, "gemma4_gpu_ids", ""))
    if gpu_ids:
        os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(gpu_ids)
        log.info("Gemma4 visible GPUs set via --gemma4-gpu-ids: %s", os.environ["CUDA_VISIBLE_DEVICES"])

    requested_device_map = str(args.gemma4_device_map).strip()
    if gpu_ids and requested_device_map in ("", "cuda:0"):
        requested_device_map = "auto" if len(gpu_ids) > 1 else "cuda:0"
    ...
    dtype = dtype_map.get(str(args.gemma4_dtype).lower())
    if dtype is None:
        raise ValueError(
            f"Unsupported --gemma4-dtype={args.gemma4_dtype}. "
            "Choose one of: float16, bfloat16, float32."
        )
    model_path = args.model_path or _DEFAULT_GEMMA4_MODEL
    if not _is_gemma4_model_path(model_path):
        log.warning(
            "Non-Gemma4 model path supplied (%s); this pipeline is Gemma4-only.",
            model_path,
        )
    ...
    if requested_device_map and requested_device_map.lower() != "none":
        pipe_kwargs["device_map"] = requested_device_map
    try:
        pipe = pipeline(**pipe_kwargs)
    except Exception as e:
        if "requires `accelerate`" not in str(e):
            raise
        fallback_device = _pipeline_device_from_map(requested_device_map)
        log.warning(
            "Gemma4 load with device_map=%s requires accelerate; falling back to device=%s",
            requested_device_map,
            fallback_device,
        )
        pipe_kwargs.pop("device_map", None)
        pipe_kwargs["device"] = fallback_device
        pipe = pipeline(**pipe_kwargs)
    ...
    log.info(
        "Text LLM backend=gemma4 model=%s device_map=%s dtype=%s gpu_ids=%s",
        model_path,
        requested_device_map,
        args.gemma4_dtype,
        ",".join(gpu_ids) if gpu_ids else "(inherited)",
    )
    return runtime, str(model_path)


def _build_gpt_final_answer_prompt(
    question: str,
    ...
    parts.extend(
        [
            "You will see frames from a short video clip again.",
            ...
            "Use BOTH what you see in the frames and the description to answer the question.",
            "If the description and the frames disagree on a visible fact, trust the frames.",
            "",
        ]
    )
...
def run_gpt_final_answer_from_clip(
    clip_path: Path,
    *,
    question: str,
    ...
    """Second multimodal pass: same clip frames + query-conditioned text -> direct answer."""
    frames_b64 = _extract_frames_as_base64(str(clip_path), fps=fps, max_frames=max_frames)
    if not frames_b64:
        raise RuntimeError(f"No frames extracted from {clip_path}")
    prompt = _build_gpt_final_answer_prompt(question, description_texts, system_instruction)
...
def compute_ordered_temporal_windows_llm_query_merge(
    ...
    *,
    ...
    raw_out = run_text_only(
        text_llm_runtime, _LLM_TIMELINE_MERGE_SYSTEM, user_msg, gen_merge
    )
...
def _extract_frames_as_base64(video_path: str, fps: int, max_frames: int) -> list[str]:
    """Extract frames from a video file and return them as base64-encoded JPEG strings."""
    import cv2
...
def describe_video_clip_with_gpt(
    video_path: str,
    gpt_client: openai.OpenAI,
    gpt_model: str,
    fps: int,
    ...
    *,
    ...
    """Send sampled frames from a video clip to GPT and return a description."""
    frames_b64 = _extract_frames_as_base64(video_path, fps=fps, max_frames=max_frames)
    if not frames_b64:
        raise RuntimeError(f"No frames extracted from {video_path}")
    prompt = build_clip_description_prompt(question, prompt_style, action_context=action_context)
    content: list[dict[str, Any]] = [{"type": "text", "text": prompt}]
    for b64 in frames_b64:
        content.append(
            {
...
def build_segment_descriptions_payload_gpt(
    question_id: str,
    video_dir: Path,
    gpt_client: openai.OpenAI,
    gpt_model: str,
    ...
    *,
    ...
    """GPT-based drop-in replacement for build_segment_descriptions_payload."""
    ...
    for video_path in video_paths:
        ...
        try:
            description = describe_video_clip_with_gpt(
                video_path,
                gpt_client,
                gpt_model,
                fps,
...
def get_predicted_actions_with_confidence_from_pkl(
    ...
    """Like get_predicted_actions_from_pkl but also records mean detection confidence per segment."""
    if video_id not in pkl_data:
        return [], None
...
def _run_single_segment_round(
    *,
    ...
    """Extract clip, Gemma tentative answer hint, then GPT query-conditioned description."""
    pred: dict[str, Any] = {
        "lookup_rank": 1,
    ...
    if "error" not in pred:
        ...
        if desc_payload is None:
            ...
            if not args.skip_llm_timeline_initial_answer and video_actions_timeline:
                try:
                    vlm_hint, llm_initial_raw = run_llm_timeline_initial_answer(
                        ...
                        w_end=end_sec_f,
                        video_actions_timeline=video_actions_timeline,
                        text_llm_runtime=text_llm_runtime,
                    ...
                    log.warning(
                        "%s [%s]: Gemma timeline initial answer failed: %s",
                        question_id,
                        round_tag,
                        _init_exc,
                    )
            ...
            desc_payload = build_segment_descriptions_payload_gpt(
                question_id,
                seg_dir,
                gpt_client,
                gpt_model,
                args.desc_fps,
                args.gpt_desc_max_frames,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            'Comma-separated GPU IDs for Gemma4 (e.g., "2,3,4"). '
            "Sets CUDA_VISIBLE_DEVICES for this process before loading Gemma."
        ),
    )
    parser.add_argument(
        "--gemma4-device-map",
        type=str,
        default="cuda:0",
        help='Device map for Gemma4 (e.g., "cuda:0", "auto").',
    )
    parser.add_argument(
        "--gemma4-dtype",
        type=str,
        ...
        help="Torch dtype used when loading Gemma4.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=384,
        help="Max new tokens for the per-question JSON timeline-merge Gemma call (text-only).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the text-only Gemma that infers a tentative answer from predicted action labels "
            "and times before the GPT description pass; use the segment justification snippet "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "timeline entries in the Gemma prompt for global context."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="OpenAI API key for GPT-based description. Falls back to OPENAI_API_KEY env var.",
    )
    parser.add_argument(
        "--gpt-vlm-model",
        type=str,
        default="gpt-4o",
        help="OpenAI model used for video clip description (default: gpt-4o).",
    )
    parser.add_argument(
        "--gpt-desc-max-frames",
        type=int,
        default=20,
        help="Max frames sent to GPT per clip (default: 20; keep low to avoid token limits).",
    )
    ...
    # Initialise the GPT client used for video clip description (VLM stage).
    ...
    gpt_model: str = args.gpt_vlm_model
    log.info("GPT VLM: model=%s  max_frames=%d", gpt_model, args.gpt_desc_max_frames)
    ...
    text_llm_runtime, model_path = init_text_llm_runtime(args, log)
    ...
    qa_instruction = (
        ...
        "you will see frames from the same short clip again together with a query-conditioned written "
        "description of that clip. Use both the frames and the description; prefer the frames if they conflict. "
    ...
    for question_id in keys:
        ...
        actions_record = {
            ...
            "model_path": model_path,
            "text_llm_model_path": text_llm_runtime["model_path"],
        ...
        while windows_queue:
            ...
            if not args.skip_confidence_refine:
                ...
                try:
                    conf_raw = run_text_only(
                        text_llm_runtime, conf_system, conf_user, gen_conf
                    )
            ...
            if should_stop:
                ...
                if mp4s_ans:
                    try:
                        final_llm_out = run_gpt_final_answer_from_clip(
                            mp4s_ans[0],
                            question=qstrip,
                            ...
                            gpt_model=gpt_model,
                            fps=args.desc_fps,
                            max_frames=args.gpt_desc_max_frames,
                            max_tokens=args.max_new_tokens_answer,
                        )
                    except Exception as _vqa_exc:
                        log.warning(
                            "%s [%s]: GPT final QA failed (%s); falling back to Gemma text-only QA.",
                            question_id,
                            round_tag,
                            _vqa_exc,
                        )
                    ...
                    log.warning(
                        "%s [%s]: no clip mp4 for GPT final QA; using Gemma text-only QA on descriptions.",
                        question_id,
                        round_tag,
                    )
                    final_llm_out = run_text_only(
        ...
        if (
            ...
            if mp4s_fb:
                try:
                    ...
                    log.warning(
                        "%s: GPT final QA (fallback) failed (%s); using Gemma text-only QA.",
                        question_id,
                        _vqa_exc_fb,
                    )
                    final_llm_out = run_text_only(
                ...
                log.warning(
                    "%s: no clip path for GPT final QA fallback; using Gemma text-only QA.",
                    question_id,
                )
                final_llm_out = run_text_only(
                    text_llm_runtime,
