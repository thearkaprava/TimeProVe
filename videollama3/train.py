...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_muon: bool = field(
        default=False,
        metadata={
            "help": "Use Muon for all 2D weight matrices (LLM, vision, projector, etc.); "
            "AdamW is only used for embeddings, lm_head, norms/biases, and other non-matrix params."
        },
    )
    muon_momentum: float = field(default=0.95, metadata={"help": "Muon internal SGD momentum."})
    muon_backend_steps: int = field(default=5, metadata={"help": "Newton–Schulz iterations in Muon."})
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            # target_modules=target_lora_modules, # LoRA on VE + LLM
