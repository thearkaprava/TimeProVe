...
# from .videollama3_qwen2_llavidal import Videollama3Qwen2ForCausalLM as Videollama3Qwen2ForCausalLM_LLAVIDAL
# from .videollama3_qwen2_llavidal import Videollama3Qwen2Config as Videollama3Qwen2Config_LLAVIDAL

# from .videollama3_qwen2_disentangled_proj import Videollama3Qwen2ForCausalLM as Videollama3Qwen2ForCausalLM_DisentangledProj
# from .videollama3_qwen2_disentangled_proj import Videollama3Qwen2Config as Videollama3Qwen2Config_DisentangledProj

# from .videollama3_qwen2_crossview_queries import Videollama3Qwen2ForCausalLM as Videollama3Qwen2ForCausalLM_crossview_queries
# from .videollama3_qwen2_crossview_queries import Videollama3Qwen2Config as Videollama3Qwen2Config_crossview_queries

VLLMs = {
    "videollama3_qwen2": Videollama3Qwen2ForCausalLM,
    # "videollama3_qwen2_llavidal": Videollama3Qwen2ForCausalLM_LLAVIDAL,
    # "videollama3_qwen2_disentangled_proj": Videollama3Qwen2ForCausalLM_DisentangledProj,
    # "videollama3_qwen2_crossview_queries": Videollama3Qwen2ForCausalLM_crossview_queries,
}

VLLMConfigs = {
    "videollama3_qwen2": Videollama3Qwen2Config,
    # "videollama3_qwen2_llavidal": Videollama3Qwen2Config_LLAVIDAL,
    # "videollama3_qwen2_disentangled_proj": Videollama3Qwen2Config_DisentangledProj,
    # "videollama3_qwen2_crossview_queries": Videollama3Qwen2Config_crossview_queries,
}
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", **kwargs):
    ...
    if 'include_visual_tokens' in config:
        MODEL_CLASS = Videollama3Qwen2ForCausalLM_crossview_queries
    else:
        MODEL_CLASS = Videollama3Qwen2ForCausalLM
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        if 'qwen2' in model_base.lower():
            model = MODEL_CLASS.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = MODEL_CLASS.from_pretrained(model_base, low_cpu_mem_usage=True, config=config, **kwargs)
        ...
        print('Loading VideoLLaMA 3 from base model, then loading tuned projector weights...')
        cfg_pretrained = PretrainedConfig.from_pretrained(model_path, token=token)
        # NOTE: AutoConfig will modify `_name_or_path` property to `model_path` if `model_path` is not None.
        # cfg_pretrained = AutoConfig.from_pretrained(model_path, token=token)
        model_base = model_base if model_base is not None else cfg_pretrained._name_or_path
        ...
        # Tuned checkpoints have a custom `model_type` / config class (e.g. `Videollama3Qwen2Config`).
        # `AutoTokenizer` relies on HF's built-in mapping for known configs, so load the tokenizer from the
        # underlying base model stored in `_name_or_path`.
        # cfg_pretrained = PretrainedConfig.from_pretrained(model_path, token=token)
        # model_base = model_base if model_base is not None else cfg_pretrained._name_or_path
        # tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, token=token)

        if model_type in ['videollama3_qwen2']:
            model = MODEL_CLASS.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
        else:
            model = MODEL_CLASS.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:
